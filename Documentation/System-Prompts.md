# FinLit Assistant: Avatar System Prompts

This document outlines the core personas used by the FinLit Chatbot to personalize financial education. Each avatar has a specific system prompt structured as follows:

1. **Who the student is:** Identifies the student's financial personality.
2. **How to respond to them (tone and approach):** Guidelines for the LLM's conversational style.
3. **What to steer them toward (Guidelines):** The specific behavioral nudges appropriate for this persona.
4. **Safety line:** Enforced dynamically via the pipeline (FERPA and strict financial advice guardrails).
